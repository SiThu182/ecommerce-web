...
import { fetchOrderDetailInfo,OrderData } from "@/api/axiosClient";
import { Variation,ProductData } from "@/api/axiosClient";
...
const CartItem: React.FC<ItemProps> = ({ item }) => {
  ...
  const totalPrice = item.variation.prices[0].sale_price * itemQuantity;
  ...
  return (
    <div className="grid grid-cols-5 items-center border-b border-gray-300 py-4 space-x-4 border my-1 rounded">
      ...
      <div className="col-span-2 flex items-center gap-4">
        <img
          src={`${
            import.meta.env.VITE_APP_BACKEND_URL + "uploads/" + item.variation.images[0].url
          }`}
        ...
        />
        <div>
          <h2 className="text-xl font-semibold">{item.variation?.product.name}</h2>
      ...
      <p className="text-sm text-gray-400">฿{item.variation.prices[0].sale_price}</p>
  ...
  const { data, error, isLoading } = useQuery<OrderData, Error>(
    {
      ...
      queryFn: () => fetchOrderDetailInfo(id),
      enabled: !!id // Only run the query if orderId is truthy (defined)
    }
  );
        ...
        <div className="flex justify-between mb-6"> 
          ...
          <div className="mb-6 w-1/2">
            <div className="flex justify-between mb-1">
              ...
              {/* <button className="p-2 bg-blue-500 rounded-lg hover:bg-gray-500 text-white">
                Invoice
              </button> */}
            </div>
            {/* <div className="flex w-72 mb-1 text-gray-600">
              <p className="w-1/2">Customer Name:</p>
              <p className="w-1/2">{data?.customer.name}</p>
            </div> */}
            <div className="flex w-72 mb-1 text-gray-600">
            ...
            <div className="flex w-72 mb-1 text-gray-600">
              <p className="w-1/2">From:</p>
              <p className="w-1/2">{new Date(data?.orderDate ?? "").toLocaleDateString()}</p>
            </div>
            <div className="flex w-72 mb-1 text-gray-600">
            ...
            <div className=" rounded-md shadow-sm p-2 flex gap-5 relative">
              ...
              <div className="">
                <div className="flex w-96 mb-2 text-gray-600">
                  ...
                  <p className="w-2/3">{data?.address?.name}</p> 
              </div>
              <div className="flex w-96 mb-2 text-gray-600">
                  <p className="w-1/3">Phone:</p>
                  <p className="w-2/3">{data?.address?.phone_no}</p> 
              </div>
              <div className="flex w-96 mb-2 text-gray-600">
                  <p className="w-1/3">Address:</p>
                  <p className="w-2/3">{data?.address.address}, {data?.address.subDistrict}, {data?.address.district}, {data?.address.province}, {data?.address.postal_code}                </p> 
              </div>
