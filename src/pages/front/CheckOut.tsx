...
const Checkout: React.FC = () => {
  ...
  const { selectedItems, shippingFees, box, specialPackage,orderType } = location.state as {
      ...
      if (activeAddress) {
        setSelectedAddress({
          ...
          address: [
            ...
            .join(", "),
          phone_no: activeAddress.phone_no,
        });
      }
    } catch (error) {
  ...
  const items = specialPackage
    ? specialPackage.items.map((pkgItem: any) => ({
        id: pkgItem.product.id,
        product: pkgItem.product,
        quantity: pkgItem.quantity,
        // If your special package items have variations/prices, add them here
        variation: pkgItem.product.variations?.[0] || {
          prices: [{ sale_price: pkgItem.product.price || specialPackage.price || 0 }],
        },
      }))
    : selectedItems || [];
  // const { data  } = useQuery({
  ...
  const handlePlaceOrder = async () => {
    ...
    formData.append("orderType", orderType || "PRODUCT");
    formData.append("shippingFees", shippingFees.toString());
    specialPackage && formData.append("packageId", specialPackage.id);
    try {
  ...
  const updateAddressStatus = () => {
    ...
    axiosClient
      .post("/customer/update-address-status", { addressId: selectedAddress })
      .then((response) => {
  ...
  <h1 className="text-2xl font-bold mb-4">
        {orderType == "PREDEFINED_PACKAGE" ? (
          <>Checkout: <span className="text-blue-600">{specialPackage.name}</span> Package</>
        ) : orderType == "CUSTOM_PACKAGE" ?  (
          <>Checkout: <span className="text-green-600">Custom Box</span></>
        ): <>Checkout</>
        }
      </h1>
